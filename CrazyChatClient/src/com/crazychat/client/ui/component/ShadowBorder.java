 ...
 * </pre>
 * 
 * Modified by RuanYaofeng on 2018-04-23<br>
 * Changes: 用于透明窗体绘制边框时, 会出现覆盖绘制变黑的情况, 加入一个标志位使其只绘制一次, 若窗体大小改变, 需重新将此标志位启用以重绘.
 * 
 * @author raistlic
 * @date Nov 10, 2012
 */
public class ShadowBorder implements Border {
    ...
    private static final float DEFAULT_PADDING_TOP = 0.5f;
    private static final float DEFAULT_PADDING_LEFT = 0.5f;
     ...
     * @return
     */
    public static ShadowBorder newInstance(boolean isPaint) {
        return new ShadowBorder(DEFAULT_SIZE, DEFAULT_COLOR, DEFAULT_ALPHA, DEFAULT_PADDING_LEFT, DEFAULT_PADDING_TOP,
                isPaint);
    }
     ...
     * 默认不绘制的ShadowBorder
     */
    private ShadowBorder(int size, Color color, float alpha, float paddingLeft, float paddingTop, boolean isPaint) {
        this(size, color, alpha, paddingLeft, paddingTop);
